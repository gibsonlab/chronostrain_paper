...
def replicate_dir(mut_ratio: str, replicate: int, base_dir: Path) -> Path:
    return base_dir / f'mutratio_{mut_ratio}' / f'replicate_{replicate}'


def trial_dir(mut_ratio: str, replicate: int, read_depth: int, trial: int, base_dir: Path) -> Path:
...
def parse_phylogroups(phylogroup_path: Path) -> Dict[str, str]:
