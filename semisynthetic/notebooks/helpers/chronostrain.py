...
def extract_chronostrain_prediction(
    ...
    target_accs, time_points, ground_truth = load_ground_truth(mut_ratio=mut_ratio, replicate=replicate, data_dir=data_dir)
...
def load_chronostrain_cluster(chronostrain_cluster: Path, phylogroup_path: Path) -> pd.DataFrame:
    df_entries = []
...
def chronostrain_results(
    ...
    posterior, strains, adhoc_clusters, runtime = extract_chronostrain_prediction(data_dir, mut_ratio, replicate, read_depth, trial, subdir_name=subdir_name)
