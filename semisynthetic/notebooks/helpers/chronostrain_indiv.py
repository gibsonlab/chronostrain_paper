...
def chronostrain_indiv_output_slice(
    ...
    trial: int,
    time_index: int,
    chronostrain_cluster_path: Path,
    ...
    posterior, strains, adhoc_clusters, runtime = extract_chronostrain_indiv_prediction(data_dir, mut_ratio, replicate, read_depth, trial, time_index, subdir_name=subdir_name)

    strain_to_idx = {s: i for i, s in enumerate(strains)}
    chronostrain_clustering_df = load_chronostrain_cluster(chronostrain_cluster_path, phylogroup_path)
...
def chronostrain_indiv_results(
    ...
    chronostrain_original_output_dir = trial_dir(mut_ratio, replicate, read_depth, trial, data_dir) / 'output' / 'chronostrain'
    try:
    ...
    for t_idx in range(len(time_points)):
        slice_A_pred_indicators, slice_A_median_traj, slice_sim_median_traj, slice_A_true_indicators, slice_A_truth, slice_sim_truth, slice_runtime = chronostrain_indiv_output_slice(
            data_dir, mut_ratio, replicate, read_depth, trial,
            t_idx,
            chronostrain_cluster_path, phylogroup_path,
