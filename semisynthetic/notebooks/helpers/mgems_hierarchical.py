...
def extract_msweep_hierarchical_prediction(
        ...
        msweep_clust_path: Path,
        phylogroup_path: Path,
        subdir_name: str = "mgems"
) -> Tuple[np.ndarray, List[str], pd.DataFrame, int]:
    cluster_df = load_msweep_ecoli_cluster(msweep_clust_path, phylogroup_path)
...
def load_msweep_ecoli_cluster(msweep_clust_path: Path, phylogroup_path: Path) -> pd.DataFrame:
    msweep_clust_df = pd.read_csv(msweep_clust_path, sep='\t')
...
def msweep_hierarchical_results(
    ...
    pred, clusters, msweep_clust_df, runtime = extract_msweep_hierarchical_prediction(
        ...
        msweep_clust_path=msweep_clust_path,
...
def msweep_hierarchical_roc(
    ...
    target_accs, time_points, ground_truth = load_ground_truth(mut_ratio=mut_ratio, replicate=replicate, data_dir=data_dir)
    pred, clusters, msweep_clust_df, runtime = extract_msweep_hierarchical_prediction(
        data_dir, mut_ratio, replicate, read_depth, trial, time_points, subdir_name=subdir_name,
        msweep_clust_path=msweep_clust_path,
        phylogroup_path=phylogroup_path
    )
    msweep_ordering = {c: i for i, c in enumerate(clusters)}
