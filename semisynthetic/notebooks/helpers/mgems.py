...
def load_msweep_cluster(db_dir: Path, phylogroup_path: Path) -> pd.DataFrame:
    ref_df = pd.read_csv(db_dir / "ref_info.tsv")

    df_entries = []
    for _, row in ref_df.iterrows():
        acc = row['id']
        clust = str(row['cluster'])
...
def msweep_roc(data_dir: Path, mut_ratio: str, replicate: int, read_depth: int, trial: int) -> Tuple[np.ndarray, np.ndarray]:
    target_accs, time_points, ground_truth = load_ground_truth(mut_ratio=mut_ratio, replicate=replicate, data_dir=data_dir)
    pred, clusters, msweep_clust_df, runtime = extract_msweep_prediction(
