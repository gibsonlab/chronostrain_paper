...
def extract_strainest_prediction(
    ...
    cluster_df = load_strainest_cluster(cluster_path, phylogroup_path)  # we ran strainest with poppunk clusters, same as in msweep.
...
def load_strainest_cluster(cluster_path: Path, phylogroup_path: Path) -> pd.DataFrame:
...
def strainest_roc(
        ...
        cluster_path: Path, phylogroup_path: Path
