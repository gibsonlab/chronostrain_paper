...
def load_ground_truth(mut_ratio: str, replicate: int, data_dir: Path) -> Tuple[List[str], List[float], np.ndarray]:
    ground_truth_path = data_dir / f"mutratio_{mut_ratio}" / f"replicate_{replicate}" / "genomes" / "abundances.txt"
