...
do
	...
	if [ -f "sorted/${tidx}_background_1.sorted.fq" ] && [ -f "sorted/${tidx}_background_2.sorted.fq" ]
	then
		...
		echo "[*] Timepoint ${tidx}: Downloading SRA entry ${sra_id}"
		download_sra $sra_id $raw_gz1 $raw_gz2

		echo "[*] Timepoint ${tidx}: Running trimmomatic processing."
		run_trimmomatic $raw_gz1 $raw_gz2 $sra_id $trimmomatic_outdir

    echo "[*] Timepoint ${tidx}: Creating background files."
    ...
    echo "[*] Timepoint ${tidx}: Re-sorting FASTQ files."
    mkdir -p sorted
    fastq-sort ${tidx}_background_1.fq -n > sorted/${tidx}_background_1.sorted.fq
    fastq-sort ${tidx}_background_2.fq -n > sorted/${tidx}_background_2.sorted.fq
	fi
done < ${BACKGROUND_CSV}
