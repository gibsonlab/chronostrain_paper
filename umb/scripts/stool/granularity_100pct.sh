...
#  -o /mnt/e/ecoli_db/chronostrain_files/ecoli.100pct.txt \
...
granular_breadcrumb=${run_dir}/inference_100pct.DONE
...
if ! [ -f $filter_breadcrumb ]; then
  ...
  export CHRONOSTRAIN_LOG_FILEPATH=${run_dir}/logs/chronostrain_inference.100pct.log
  export CHRONOSTRAIN_CACHE_DIR=${run_dir}/chronostrain/cache
  echo "[*] Logging output to: ${CHRONOSTRAIN_LOG_FILEPATH}"
  ...
  chronostrain advi \
    -r ${run_dir}/filtered/filtered_reads.csv \
    -o ${run_dir}/chronostrain_100pct \
    -s /mnt/e/ecoli_db/chronostrain_files/ecoli.100pct.txt \
