...
class ChronostrainRenderer:
    ...
    def plot_overall_relabund(
        ...
        for s_idx in pd.unique(df['StrainIdx']):
            ...
            for _t, _L, _U, _M in zip(res.time_points, lower, upper, median):
                plotted_data.append({
                    'StrainId': s_id, 'Time': _t,
                    'AbundLower': _L, 'AbundUpper': _U, 'AbundMedian': _M
                })
        ax.set_yscale(yscale)
        ax.set_xticks(res.time_points)
        return ymin, ymax, pd.DataFrame(plotted_data)
